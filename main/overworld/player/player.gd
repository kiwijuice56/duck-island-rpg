...
func _physics_process(_delta):
	...
	if get_tree().get_root().has_node("Main/Overworld/Beach/WaterTiles") and not get_tree().get_root().get_node("Main/Overworld/Beach/WaterTiles").get_cell( snapped.x, snapped.y)  == -1:
